...
- **Skills 自动更新**：本文件及其 skills 库托管于 GitHub（`github.com:Lixy-XDU/claude-skills`）。运行 `/skill-updater` 即可一键检查并拉取最新 skills，自动备份旧 CLAUDE.md、重建关系图、不覆盖 `config.local.yaml`。建议定期运行或在重要任务前运行。  
