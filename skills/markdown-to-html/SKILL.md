...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `.claude/skills/markdown-to-html/EXTEND.md` | Project |
| 2 | `$HOME/.claude/skills/markdown-to-html/EXTEND.md` | User |
...
| `--help` | Show help | |
| `--out <dir>` | Output directory (creates if not exists) | Same dir as input |
**Color Presets:**
...
**File location**: Default same as input; use `--out <dir>` to override.
- Input: `/path/to/article.md`
- Output: `/path/to/article.html` (or `<out>/article.html` with `--out`)
...
## LaTeX Formula Protection (Critical Rule)

**公式必须先保护再转换。** 转换引擎在 Markdown → HTML 之前，使用占位符机制保护所有 LaTeX 公式和 Obsidian wiki link，防止 Markdown parser 和 HTML sanitizer 破坏它们。
...
转换完成后，占位符被恢复为原始内容。MathJax v3 自动注入到 `<head>`。

### 严禁的操作

以下代码或等价逻辑会导致 LaTeX 公式损坏，严格禁止：
- `replace("\\,", ",")` — 破坏 `\,` 空格命令
- `replace("\\\\", "\\")` — 破坏换行符 `\\`
- `replace("\\|", "|")` — 破坏范数符号 `\|`
- `re.sub(r"\\(.)", r"\1", text)` — 通用去反斜杠
- 在 `$$` 或 `$` 块内应用 Markdown 斜体/粗体解析
